...
if (browser && localStorage != null) {
  lastUsername.subscribe((newUsername) => {
    ...
    console.log(
      `LocalStorage: SetItem '${lastUsernameKey}' to '${newUsername}'`,
    );
  });
}
...
const lastChatroomKey: string = "chatroom";
export const lastChatroom: Writable<string> = writable<string>(
  browser && localStorage != null
    ? (localStorage.getItem(lastChatroomKey) ?? "")
    : "",
);
if (browser && localStorage != null) {
  lastChatroom.subscribe((newChatroomName) => {
    ...
    console.log(
      `LocalStorage: SetItem '${lastChatroomKey}' to '${newChatroomName}'`,
    );
  });
}
...
export const inChatroom: Readable<boolean> = derived<
