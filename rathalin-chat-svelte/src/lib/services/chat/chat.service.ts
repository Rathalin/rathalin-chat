...
class ChatService {
    ...
    public login(username: Username): Promise<boolean> {
        return new Promise((resolve) => {
            this._socket.on(ServerEvent.RESPONSE_LOGIN_USERNAME_ACCEPT, () => {
                resolve(true);
                this._socket.removeAllListeners(ServerEvent.RESPONSE_LOGIN_USERNAME_ACCEPT);
            });
            this._socket.on(ServerEvent.RESPONSE_LOGIN_USERNAME_TAKEN, () => {
                resolve(false);
                this._socket.removeAllListeners(ServerEvent.RESPONSE_LOGIN_USERNAME_TAKEN);
            });
    ...
    public chatroomExists(room: Chatroom): Promise<boolean> {
        return new Promise((resolve) => {
            ...
            this._socket.on(ServerEvent.RESPONSE_CHATROOM_NOT_EXISTS, () => {
                resolve(false);
                this._socket.removeAllListeners(ServerEvent.RESPONSE_CHATROOM_NOT_EXISTS);
            });
    ...
    public requestOnlineUsers(): Promise<UsernameListMessage> {
        return new Promise((resolve) => {
            this._socket.on(ServerEvent.RESPONSE_ONLINE_USERS, (onlineUsernameListMessage: UsernameListMessage) => {
    ...
    private initConnections(): void {
        ...
        // Text Message
        this._socket.on(ServerEvent.SEND_TEXT_MESSAGE, (textMessage: TextMessage): void => {
            this.onTextMessage.next(textMessage);
        });

        // System
        this._socket.on(ServerEvent.SEND_SYSTEM, (systemMessage: SystemMessage): void => {
