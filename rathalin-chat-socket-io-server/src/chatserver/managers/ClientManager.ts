...
export class ClientManager {
  ...
  public getClientBySocket(socket: Socket): Client {
    const client: Client | undefined = this._clients.find(
      (c) => c.socket === socket,
    );
    if (client == null) {
  ...
  public addChatroom(name: ChatroomName): void {
    ...
    if (this.chatroomExists(name)) {
      throw new Error(
        `Chatroom '${name}' can not be added because it already exists.`,
      );
    }
  ...
  public addMessagesToChatroom(
    ...
    if (room.messages.length > this.messageLimitPerChatroom) {
      room.messages.splice(
        0,
        room.messages.length - this.messageLimitPerChatroom,
      );
    }
  }
  ...
  public isClientInChatroom(socket: Socket, roomName: ChatroomName): boolean {
    const client: Client = this.getClientBySocket(socket);
    return this._clientsInChatrooms.some(
      (clientInRoom) =>
        clientInRoom.client === client && clientInRoom.roomName === roomName,
    );
  }
  ...
  public removeClientFromChatroom(
    socket: Socket,
    roomName: ChatroomName,
  ): void {
    const client: Client = this.getClientBySocket(socket);
    this._clientsInChatrooms = this._clientsInChatrooms.filter(
      (clientInRoom) =>
        clientInRoom.client !== client || clientInRoom.roomName !== roomName,
    );
  }
  ...
  private getChatroomByName(name: ChatroomName): Chatroom {
    const foundChatroom: Chatroom | undefined = this._chatrooms.find(
      (c) => c.name === name,
    );
  ...
  private removeClientFromChatrooms(socket: Socket): void {
    const client: Client = this.getClientBySocket(socket);
    this._clientsInChatrooms = this._clientsInChatrooms.filter(
      (clientInRoom) => clientInRoom.client !== client,
    );
  }

  private removeChatroomFromClients(roomName: ChatroomName): void {
    this._clientsInChatrooms = this._clientsInChatrooms.filter(
      (clientInRoom) => clientInRoom.roomName !== roomName,
    );
  }
