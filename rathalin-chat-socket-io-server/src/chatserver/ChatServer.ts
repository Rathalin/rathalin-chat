...
export class ChatServer {
  ...
  public listen(socketIOPort: number, corsPort: number | "*"): void {
    ...
    const corsOrigin: string =
      corsPort === "*" ? corsPort : `http://localhost:${corsPort}`;
  ...
  private initConnections(): void {
    if (this.server == null)
      throw new Error("Initialize Server by calling 'listen'!");

    this.server.on("connection", (socket) => {
      this.clientManager.addClient(socket);
      this.logger.log(
        `+ User (${this.clientManager.numberOfClients})`,
        "connect",
      );
  ...
  private registerClientDisconnect(socket: Socket): void {
    socket.on("disconnect", (reason) => {
      ...
      this.logger.log(
        `- User (${this.clientManager.numberOfClients - 1}) (Reason: ${reason})`,
        "disconnect",
      );
      if (client.user?.username?.trim()) {
        ...
        const roomesOfClient: string[] =
          this.clientManager.getChatroomNamesOfClient(socket);
        if (roomesOfClient.length > 0) {
          socket
            .to(roomesOfClient)
            .emit(ServerEvent.SEND_LEAVE_CHATROOM, userLeavingChatroomMessage);
        }
      }
  ...
  private registerClientSendsTextMessage(
    socket: Socket,
    roomName: ChatroomName,
  ): void {
    socket.on(
      ClientEvent.SEND_TEXT_MESSAGE,
      (clientTextMessage: TextMessage) => {
        ...
        socket.broadcast
          .to(roomName)
          .emit(ServerEvent.SEND_TEXT_MESSAGE, textMessage);
  ...
  private registerClientRequestsMessageList(
    socket: Socket,
    roomName: ChatroomName,
  ): void {
  ...
  private registerClientRequestsOnlineUserList(
    socket: Socket,
    roomName: ChatroomName,
  ): void {
  ...
  private registerClientRequestsCreateChatroom(socket: Socket): void {
    socket.on(
      ClientEvent.REQUEST_CREATE_CHATROOM,
      (chatroomMessage: ChatroomMessage) => {
        if (!this.clientManager.authUser(socket)) return;
        ...
        if (this.clientManager.chatroomLimitReached()) {
          ...
          this.logger.log(
            `Chatroom limit of ${this.chatroomLimit} reached.`,
        ...
        if (this.clientManager.chatroomExists(roomName)) {
          ...
          this.logger.log(
            `Chatroom "${chatroomMessage.room}" not created because taken.`,
        ...
        this.logger.log(
          `Chatroom "${chatroomMessage.room}" was created.`,
  ...
  private registerClientRequestsJoinChatroom(socket: Socket): void {
    socket.on(
      ClientEvent.REQUEST_JOIN_CHATROOM,
      (chatroomMessage: ChatroomMessage) => {
        if (!this.clientManager.authUser(socket)) return;
        const roomName: ChatroomName = chatroomMessage.room
          .trim()
          .substring(0, this.roomNameMaxLength);
        ...
        if (this.clientManager.chatroomExists(roomName)) {
          ...
          this.logger.log(
            `Join chatroom "${chatroomMessage.room}" accept.`,
          ...
          const userJoiningChatroomMessage: UsernameMessage = {
            ...
            date: new Date().toString(),
            username:
              this.clientManager.getClientBySocket(socket).user.username,
          };
          socket.broadcast
            .to(roomName)
            .emit(ServerEvent.SEND_JOIN_CHATROOM, userJoiningChatroomMessage);
          this.logger.log(
            `'${userJoiningChatroomMessage.username}' is joining '${roomName}'`,
            ServerEvent.SEND_JOIN_CHATROOM,
          );
        } else {
          socket.emit(ServerEvent.RESPONSE_JOIN_CHATROOM_NOT_EXISTING);
          this.logger.log(
            `Join chatroom "${chatroomMessage.room}" not accepted, because not existing.`,
  ...
  private registerClientRequestsLeaveChatroom(socket: Socket): void {
    socket.on(
      ClientEvent.REQUEST_LEAVE_CHATROOM,
      (chatroomMessage: ChatroomMessage) => {
        if (!this.clientManager.authUser(socket)) return;
        const roomName: ChatroomName = chatroomMessage.room.trim();
        if (this.clientManager.isClientInChatroom(socket, roomName)) {
          ...
          this.logger.log(
            `Leave chatroom "${chatroomMessage.room}" accept.`,
            ServerEvent.RESPONSE_LEAVE_CHATROOM_ACCEPT,
          );
          const userLeavingChatroomMessage: UsernameMessage = {
            event: ServerEvent.SEND_LEAVE_CHATROOM,
            type: MessageType.USERNAME,
          ...
          socket.broadcast
            ...
            .emit(ServerEvent.SEND_LEAVE_CHATROOM, userLeavingChatroomMessage);
          this.logger.log(
            `'${userLeavingChatroomMessage.username}' is leaving '${roomName}'`,
            ServerEvent.SEND_LEAVE_CHATROOM,
          );
        } else {
          socket.emit(ServerEvent.RESPONSE_LEAVE_CHATROOM_NOT_JOINED);
          this.logger.log(
            `Leave chatroom "${chatroomMessage.room}" not accept, because not joined!`,
            ServerEvent.RESPONSE_LEAVE_CHATROOM_NOT_JOINED,
          );
        }
      },
    );
  ...
  private sendMessageListToClient(
    socket: Socket,
    roomName: ChatroomName,
    limit?: number,
  ): void {
    ...
    const existingMessage: Message[] =
      this.clientManager.getMessagesOfChatroom(roomName);
    if (limit != null) {
      messages = existingMessage.slice(
        Math.max(0, existingMessage.length - limit),
        existingMessage.length,
      );
    } else {
      messages = existingMessage;
    }
    const allTextMessages: TextMessage[] = messages.filter(
    ...
    const messageListMessage: MessageListMessage = {
      ...
      date: new Date().toString(),
      messages: [...allTextMessages, loginMessageOfClient],
    };
    this.logger.log(
      `MessageList(${messageListMessage.messages.length})`,
      ServerEvent.RESPONSE_MESSAGE_LIST,
    );
    socket.emit(ServerEvent.RESPONSE_MESSAGE_LIST, messageListMessage);
  }

  private sendOnlineUsersToClient(
    socket: Socket,
    roomName: ChatroomName,
  ): void {
    const onlineUsersMessage: UsernameListMessage = {
      ...
      date: new Date().toString(),
      users: this.clientManager
        .getClientsOfChatroom(roomName)
        .map((c) => c.user.username)
        .filter((name) => name !== ""),
    };
    this.logger.log(
      `Online users (${onlineUsersMessage.users.length})`,
